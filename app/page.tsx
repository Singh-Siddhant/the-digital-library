...
export default function Home() {
  ...
  // Redirection when user is already logged in
  useEffect(() => {
    if (!authLoading && user) {
      router.push('/explore');
    }
  ...
  // Initialize Google Identity Services (GSI) Button
  useEffect(() => {
    ...
    const initializeGSI = () => {
      if ((window as any).google?.accounts?.id) {
        try {
          ...
          const btnParent = document.getElementById("googleBtnHome");
          if (btnParent) {
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center min-h-[56px] w-full">
          {loadingLogin ? (
            ...
            <div id="googleBtnHome" className="w-full flex justify-center" />
          )}
        </div>
